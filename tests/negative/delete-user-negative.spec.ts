...
test.describe('DELETE User Negative Tests', () => {
  /**
   * Validates: Requirements 8.8
   * Test: DELETE with invalid user ID format returns 400
   */
  test('should return 400 for invalid ID format', async ({ api }) => {
    const response = await api.delete('/api/users/abc');
   ...
   * Test: DELETE with non-existing numeric user ID returns 404
   */
  test('should return 404 for non-existing numeric user ID', async ({ api }) => {
    const response = await api.delete('/api/users/999999');
